...
**Modified Data Vault**
1. output the counter for each file. Key terms are: get_counter, datacounter, get_counter_from_session
2. data format %05d -- not yet
"""
...
class Session( object ):
    ...
    def newDataset( self, title, independents, dependents, dtype ):
        ...
        return dataset
    def get_counter_from_session(self):
        return self.counter
...
class Dataset:
    ...
    def addParameter( self, name, data, saveNow = True ):
        ...
        self.parent.onNewParameterDataset((int(self.name[0:5]), self.name[8:len(self.name)], self.session.path, name), self.parent.root.listeners) ###fixed due to data length
        self.param_listeners = set()
        return name
...
class DataVault( LabradServer ):
    ...
    def getSession( self, c ):
        ...
        session = Session( c['path'], self )
        self.datacounter = session.get_counter_from_session()
        return session
    ...
    @setting(333, 'get counter', returns = 'i')
    ...
    def newMatrixImgset(self, title, size, dtype):
        num = self.counter
        self.counter += 1
        self.modified = datetime.now()
        name = '%05d_%s' % (num, title)
