...
class switchWidget(QtGui.QFrame):
    ...
    def initializeGUI(self, channels):
        ...
        for order,name in enumerate(channels):
            ...
            buttonSeq = QtGui.QPushButton('Seq')
            buttonSeq.setCheckable(True)
            buttonSeq.setAutoExclusive(True)
            groupBoxLayout.addWidget(buttonOn)
            groupBoxLayout.addWidget(buttonOff)
            groupBoxLayout.addWidget(buttonAuto)
            groupBoxLayout.addWidget(buttonSeq)
            ...
            buttonSeq.clicked.connect(self.buttonConnectionSeq(name,server))
    ...
    def buttonConnectionAuto(self, name, server):
        ...
        return func
    def buttonConnectionSeq(self,name,server):
        from labrad.units import WithUnit as U
        @inlineCallbacks
        def func(state):
            yield server.new_sequence()
            yield server.add_ttl_pulse(name,U(100,'ms'),U(50,'ms'))
            yield server.program_sequence()
            yield server.start_number(5)
        return func
